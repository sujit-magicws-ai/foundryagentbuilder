...
> **Two-SDK Architecture**: `AIProjectClient` (`azure-ai-projects`) handles Foundry management (agent definitions, connections, deployments, datasets, indexes, evaluations). `AgentsClient` (`azure-ai-agents`) handles agent runtime (create_agent, threads, messages, runs, streaming, function calls). `AIProjectClient.agents` only has definition CRUD: `create`, `create_version`, `get`, `list`, `update`, `delete`.

### AIProjectClient (Sync) — Foundry Management
...
client.agents          # AgentsOperations — definition CRUD (create, create_version, get, list, update, delete)
...
async with AIProjectClient(
    ...
    # AIProjectClient.agents has definition CRUD: create_version, get, list, delete
    # For runtime operations (create_agent, threads, messages, runs), use AgentsClient from azure.ai.agents
    agent_def = await client.agents.create_version(...)
```

### AgentsClient (Sync) — Agent Runtime

> **Package**: `azure-ai-agents` (separate SDK from `azure-ai-projects`)
...
# Runtime operations
agents_client.create_agent(...)       # Create live agent
agents_client.delete_agent(...)       # Delete agent
agents_client.enable_auto_function_calls(...)  # Auto function calling
agents_client.threads.create()        # Thread management
agents_client.messages.create(...)    # Message management
agents_client.runs.create_and_process(...)  # Run execution
agents_client.runs.stream(...)        # Streaming runs
```
