...
Tools are used with both SDKs:
- **AgentsClient** (`azure-ai-agents`) — Runtime agent creation with tools
- **AIProjectClient** (`azure-ai-projects`) — Versioned agent definitions with project-level tools

## Tool Import Patterns

```python
# From azure.ai.agents.models (runtime tools)
...
# From azure.ai.projects.models (project-level tools for versioned definitions)
...
### Explicit Function Definition (for versioned agents)
...
### Using Low-Level Tool (AgentsClient)
...
### Using Project-Level Tool (AIProjectClient versioned definition)
...
Enterprise search over your Azure AI Search indexes (project-level tool).
...
# Get search connection (AIProjectClient)
...
Combine multiple tools with auto-execution (AgentsClient only):
...
| Tool | Class | Import From | Connection Required | Use Case |
|------|-------|-------------|---------------------|----------|
| Code Interpreter | `CodeInterpreterTool` | `azure.ai.agents.models` | No | Execute Python, generate files |
| File Search | `FileSearchTool` | `azure.ai.agents.models` | No | RAG over uploaded documents |
| Function | `FunctionTool` | `azure.ai.agents.models` | No | Call custom Python functions |
| Bing Grounding | `BingGroundingTool` | `azure.ai.agents.models` | Yes | Web search (runtime) |
| Bing Grounding | `BingGroundingAgentTool` | `azure.ai.projects.models` | Yes | Web search (versioned def) |
| Azure AI Search | `AzureAISearchAgentTool` | `azure.ai.projects.models` | Yes | Enterprise search |
| OpenAPI | `OpenApiTool` | `azure.ai.agents.models` | No | Call REST APIs |
| MCP | `McpTool` | `azure.ai.agents.models` | No | MCP server integration |
| SharePoint | `SharepointTool` | `azure.ai.agents.models` | Yes | SharePoint search |
| Fabric | `FabricTool` | `azure.ai.agents.models` | Yes | Microsoft Fabric integration |
| Connected Agent | `ConnectedAgentTool` | `azure.ai.agents.models` | No | Multi-agent orchestration |

## Which Client for Which Tool?

| Tool Pattern | Client | Method |
|-------------|--------|--------|
| Runtime agent with tools | `AgentsClient` | `agents_client.create_agent(tools=..., toolset=...)` |
| ToolSet with auto-execution | `AgentsClient` | `agents_client.enable_auto_function_calls(toolset)` |
| Versioned agent definition with tools | `AIProjectClient` | `project_client.agents.create_version(definition=PromptAgentDefinition(tools=...))` |
| File upload | `AgentsClient` | `agents_client.files.upload_and_poll(...)` |
| Vector store creation | `AgentsClient` | `agents_client.vector_stores.create_and_poll(...)` |
| Get connection for tool config | `AIProjectClient` | `project_client.connections.get(...)` |
