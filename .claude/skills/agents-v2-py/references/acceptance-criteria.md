...
## 9. SDK Architecture Awareness

### 9.1 Understanding the Two SDKs

Hosted agent definitions use `AIProjectClient` from `azure-ai-projects`. This is DIFFERENT from `AgentsClient` in `azure-ai-agents`:

| Operation | SDK | Client | Method |
|-----------|-----|--------|--------|
| Create agent definition | `azure-ai-projects` | `AIProjectClient` | `client.agents.create_version()` |
| List/get/delete definitions | `azure-ai-projects` | `AIProjectClient` | `client.agents.list()`, `.get()`, `.delete_version()` |
| Run agents (threads/messages) | `azure-ai-agents` | `AgentsClient` | `agents_client.threads.*`, `.messages.*`, `.runs.*` |
| Upload files / vector stores | `azure-ai-agents` | `AgentsClient` | `agents_client.files.*`, `.vector_stores.*` |

### 9.2 Anti-Patterns (ERRORS)

#### ❌ INCORRECT: Expecting runtime operations on AIProjectClient

```python
# WRONG - AIProjectClient.agents does NOT have threads, messages, runs
client = AIProjectClient(endpoint=endpoint, credential=credential)
thread = client.agents.threads.create()  # DOES NOT EXIST
client.agents.messages.create(...)       # DOES NOT EXIST
client.agents.runs.create_and_process(...)  # DOES NOT EXIST
```

#### ❌ INCORRECT: Using AgentsClient for hosted agent definitions

```python
# WRONG - AgentsClient does NOT have create_version
from azure.ai.agents import AgentsClient

agents_client = AgentsClient(endpoint=endpoint, credential=credential)
agents_client.create_version(...)  # DOES NOT EXIST - use AIProjectClient
```
...
- [ ] NOT using `AIProjectClient.agents` for runtime operations (threads, messages, runs)
- [ ] NOT using `AgentsClient` for hosted agent definitions
