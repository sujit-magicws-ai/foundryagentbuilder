...
This skill uses `AIProjectClient` from `azure-ai-projects` for agent **definition management** (CRUD operations). This is correct — `client.agents.create_version()` creates agent definitions in Azure AI Foundry.

| SDK | Package | Client | Purpose |
|-----|---------|--------|---------|
| **Azure AI Projects** (this skill) | `azure-ai-projects` | `AIProjectClient` | Foundry management: agent definitions, connections, deployments |
| **Azure AI Agents** (runtime) | `azure-ai-agents` | `AgentsClient` | Agent runtime: create_agent, threads, messages, runs, streaming |
| **Agent Framework** (high-level) | `agent-framework-azure-ai` | `AzureAIAgentsProvider` | High-level agent creation, run, streaming |

**Key points:**
- `client.agents` is `AgentsOperations` — it only has definition CRUD: `create`, `create_version`, `get`, `list`, `delete`, `update`, `list_versions`, `delete_version`
- Do NOT use `AgentsClient` for hosted agent definitions — use `AIProjectClient`
- Do NOT use `AIProjectClient.agents` for runtime operations (threads, messages, runs) — it doesn't have them
- Hosted agents run in containers; to interact with them after deployment, use the agent's deployed endpoint
