...
## SDK Architecture (Three SDKs)

This skill uses the **Microsoft Agent Framework** — a high-level SDK that wraps the lower-level Azure SDKs:

| SDK | Package | Client | Purpose |
|-----|---------|--------|---------|
| **Agent Framework** (this skill) | `agent-framework-azure-ai` | `AzureAIAgentsProvider` | High-level agent creation, run, streaming, threads |
| **Azure AI Agents** (runtime) | `azure-ai-agents` | `AgentsClient` | Low-level agent runtime (file uploads, vector stores) |
| **Azure AI Projects** (management) | `azure-ai-projects` | `AIProjectClient` | Foundry management (connections, deployments, agent definitions) |

**Key points:**
- `AzureAIAgentsProvider` internally wraps `AgentsClient` from `azure-ai-agents`
- For file uploads and vector stores, use `AgentsClient` directly (see tools.md)
- Do NOT use `AIProjectClient.agents` for runtime operations — it only has definition CRUD (`create_version`, `get`, `list`, `delete`)
- Do NOT confuse `provider.create_agent()` (Agent Framework) with `agents_client.create_agent()` (azure-ai-agents) — they are different SDKs
